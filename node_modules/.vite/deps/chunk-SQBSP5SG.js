...
var logger9 = new ConsoleLogger("StorageCache");
var StorageCacheCommon = class {
  ...
  configure(config2) {
    if (config2) {
      if (config2.keyPrefix) {
        logger9.warn("keyPrefix can not be re-configured on an existing Cache instance.");
      }
      this.config = {
        ...this.config,
        ...config2
  ...
  async setItem(key, value, options) {
    logger9.debug(`Set item: key is ${key}, value is ${value} with options: ${options}`);
    if (!key || key === currentSizeKey) {
      logger9.warn(`Invalid key: should not be empty or reserved key: '${currentSizeKey}'`);
      return;
    }
    if (typeof value === "undefined") {
      logger9.warn(`The value of item should not be undefined!`);
      return;
    }
    ...
    if (cacheItemOptions.priority < 1 || cacheItemOptions.priority > 5) {
      logger9.warn(`Invalid parameter: priority due to out or range. It should be within 1 and 5.`);
      return;
    }
    const prefixedKey = `${this.config.keyPrefix}${key}`;
    const item = this.fillCacheItem(prefixedKey, value, cacheItemOptions);
    if (item.byteSize > this.config.itemMaxSize) {
      logger9.warn(`Item with key: ${key} you are trying to put into is too big!`);
      return;
    }
    try {
  ...
  async getItem(key, options) {
    logger9.debug(`Get item: key is ${key} with options ${options}`);
    let cached;
    if (!key || key === currentSizeKey) {
      logger9.warn(`Invalid key: should not be empty or reserved key: '${currentSizeKey}'`);
      return null;
   ...
   */
  async removeItem(key) {
    logger9.debug(`Remove item: key is ${key}`);
    if (!key || key === currentSizeKey) {
      logger9.warn(`Invalid key: should not be empty or reserved key: '${currentSizeKey}'`);
      return;
    }
    ...
    try {
      ...
      logger9.warn(`removeItem failed! ${e}`);
    }
  }
  /**
  ...
  async getAllKeys() {
    try {
      ...
      logger9.warn(`getAllkeys failed! ${e}`);
      return [];
    }
  }
  getStorage() {
  ...
  async removeCacheItem(prefixedKey, size) {
    ...
    try {
      ...
      logger9.error(`Failed to remove item: ${removeItemError}`);
    }
  }
  /**
  ...
  sanitizeConfig() {
    if (this.config.itemMaxSize > this.config.capacityInBytes) {
      logger9.error("Invalid parameter: itemMaxSize. It should be smaller than capacityInBytes. Setting back to default.");
      this.config.itemMaxSize = defaultConfig.itemMaxSize;
    }
    if (this.config.defaultPriority > 5 || this.config.defaultPriority < 1) {
      logger9.error("Invalid parameter: defaultPriority. It should be between 1 and 5. Setting back to default.");
      this.config.defaultPriority = defaultConfig.defaultPriority;
    }
    if (Number(this.config.warningThreshold) > 1 || Number(this.config.warningThreshold) < 0) {
      logger9.error("Invalid parameter: warningThreshold. It should be between 0 and 1. Setting back to default.");
      this.config.warningThreshold = defaultConfig.warningThreshold;
    }
    const cacheLimit = 5 * 1024 * 1024;
    if (this.config.capacityInBytes > cacheLimit) {
      logger9.error("Cache Capacity should be less than 5MB. Setting back to default. Setting back to default.");
  ...
  async setCacheItem(prefixedKey, item) {
    ...
    try {
      ...
      logger9.error(`Failed to set item ${setItemErr}`);
    }
  }
  /**
  ...
  async clear() {
    ...
    try {
      ...
      logger9.warn(`clear failed! ${e}`);
    }
  }
};

// node_modules/@aws-amplify/core/dist/esm/Cache/StorageCache.mjs
var logger10 = new ConsoleLogger("StorageCache");
var StorageCache = class _StorageCache extends StorageCacheCommon {
  ...
  createInstance(config2) {
    if (!config2.keyPrefix || config2.keyPrefix === defaultConfig.keyPrefix) {
      logger10.error("invalid keyPrefix, setting keyPrefix with timeStamp");
...
//# sourceMappingURL=chunk-SQBSP5SG.js.map
